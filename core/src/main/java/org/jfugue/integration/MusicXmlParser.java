...
/**
 * Parses a MusicXML file, and fires events for <code>ParserListener</code> interfaces when tokens
 * are interpreted. The <code>ParserListener</code> does intelligent things with the resulting
 * events, such as create music, draw sheet music, or transform the data.
 *
...
public final class MusicXmlParser extends Parser {
  ...
  private static final Map<String, String> XMLtoJFchordMap;

  static {
    // @formatter:off
    XMLtoJFchordMap = new TreeMap<>(CHORD_COMPARATOR);
    // Major Chords
  ...
  private final byte curVelocity = DefaultNoteSettingsManager.getInstance().getDefaultOnVelocity();
  ...
  // CONSTRUCTOR
  /**
   * <p>Constructor for MusicXmlParser.</p>
   */
  ...
  /**
   * converts beats per minute (BPM) to pulses per minute (PPM) assuming 240 pulses per second In
   * MusicXML, BPM can be fractional, so
  ...
  public static int BPMtoPPM(float bpm) {
    return (int) ((60.f * 240.f) / bpm);
  }

  /**
   * <p>parse.</p>
   *
   * @param musicXmlString a {@link java.lang.String} object.
   * @throws nu.xom.ParsingException if any.
   * @throws java.io.IOException if any.
   */
  public void parse(String musicXmlString) throws
    ParsingException, IOException {
    // URI is null when parsing a String as it's coming from somewhere else
    parse(xomBuilder.build(musicXmlString, null));
  }
   ...
   * <p>parse.</p>
   *
   * @param inputStream a {@link java.io.FileInputStream} object.
   * @throws nu.xom.ParsingException if any.
   * @throws java.io.IOException if any.
   */
  public void parse(FileInputStream inputStream) throws
    ParsingException, IOException {
   ...
   * @throws java.io.IOException if any.
   */
  public void parse(Reader reader) throws
    ParsingException, IOException {
   ...
   * sent to <code>ParserListener</code> interfaces, which are responsible for doing something
  ...
  private Map<String, PartContext> parsePartList(Elements parts) {
    Map<String, PartContext> partHeaders = new HashMap<>();
  ...
  private PartContext parsePartHeader(Element part) {
    ...
    PartContext partHeader = new PartContext(part.getAttribute("id").getValue(),
      part.getFirstChildElement("part-name").getValue());
    ...
    for (int x = 0; x < midiInsts.size(); ++x) {
      ...
      byte program = getByteValueOrDefault(midi_instrument.getFirstChildElement("midi-program"),
        (byte) 0);
      String unpitched = getStringValueOrNull(
        midi_instrument.getFirstChildElement("midi-unpitched"));
      partHeader.instruments[x] = new MidiInstrument(instrumentId, channel, name, bank, program,
        unpitched);
    }
  ...
  /**
   * Parses music data in under either a measure for timewise score or under a part for partwise
   * scores.
   *
   * @param partIndex Index of the part
   */
  private void parseMusicData(int partIndex, String partId, Map<String, PartContext> partHeaders,
    Element musicDataRoot) {
    ...
    if (attributes != null) {
      ...
      //Time-Signature
      this.divisionsPerBeat = getByteValueOrDefault(attributes.getFirstChildElement("divisions"),
        this.divisionsPerBeat);
      this.beatsPerMeasure = getByteValueOrDefault(
        getRecursiveFirstChildElement(attributes, "time", "beats"), this.beatsPerMeasure);
    }
  ...
  private void parseNote(int p, Element noteElement, String partId,
    ...
    for (int i = 0; i < noteEls.size(); i++) {
      ...
      switch (tagName) {
        case "instrument":
          ...
          for (MidiInstrument ins : instruments) {
        ...
        case "pitch":
          ...
          octaveNumber = getByteValueOrDefault(element.getFirstChildElement("octave"),
            octaveNumber);
    ...
    if (isRest) {
      ...
      newNote.setOffVelocity(this.curVelocity);
    }
   ...
   *
   * @param name is a String that represents the instrument. If it is a numeric value, it is
   * interpreted as a midi-bank or program. If it is an instrument name, it is looked up in the
   * Dictionary as an instrument name.
   */
  private void parseInstrumentNameAndFireChange(String name) {
    ...
    try {
      ...
      instrumentNumber = (value == null) ? -1 : value;
    }
  ...
  private void parseInstrumentAndFireChange(MidiInstrument instrument) {
    if (instrument.program >= 0) {
      ...
      throw new RuntimeException(
        "Couldn't determine the instrument. Possibly and unhandled case. Please report with the musicXML data.");
    }
  }
  ...
  private static class MidiInstrument {
    ...
    MidiInstrument(String id, String channel, String name, String bank, byte program,
      String unpitched) {
      this.id = id;
  ...
  private static class VoiceDefinition {
    ...
    VoiceDefinition(int part, int voice) {
      this.part = part;
      this.voice = voice;
    }
  }
  ...
  private static class KeySignature {
    ...
    KeySignature(byte key, byte scale) {
      this.key = key;
      this.scale = scale;
    }
