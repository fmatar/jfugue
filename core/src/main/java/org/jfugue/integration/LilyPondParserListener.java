...
/**
 * This class listens to events from the MusicString parser. In response to this events, a Lilypond
 * string is produced. The Lilypond string is produced with relative octave notation.
 *
...
public class LilyPondParserListener extends ParserListenerAdapter {
  ...
  public void onInstrumentParsed(byte instrument) {
    ...
    String instrument2 =
      "\\set Staff.instrumentName = \"" + MidiDictionary.INSTRUMENT_BYTE_TO_STRING
        .get(Byte.parseByte(id)) + "\" ";
  ...
  private void printChord(Chord chord) {
    log("Chord parsed: rootnote = " + chord.getRoot().getValue() + "  intervals = " + chord
      .getIntervals().toString() + "  duration = " + chord.getRoot().getDuration() + "  attack = "
  ...
  public void onChordParsed(Chord chord) {
    ...
    String duration = LilyPondNoteDurationHelper
      .getDuration2(Double.toString(chord.getRoot().getDuration()));
  ...
  private void save(boolean isLastSave) {
    ...
    if (!isLastSave) {
      ...
      if (noteWindow.getSecondPreviousNote().isFirstNote() && noteWindow.getPreviousNote()
        .isHarmonicNote() && !noteWindow.getCurrentNote().isMelodicNote()) {
        lyString.append("<");
      ...
      } else if (noteWindow.getSecondPreviousNote().isHarmonicNote() && noteWindow.getPreviousNote()
        .isFirstNote()) {
        // close parallel
        lyString.append(noteWindow.getSecondPreviousNoteLy());
        lyString.append(">");
    ...
    if (!isLastSave && noteWindow.getPreviousNote() != null && noteWindow.getPreviousNote()
      .isFirstNote() && noteWindow.getCurrentNote().isHarmonicNote()) {
      log("We don't know yet");
      return;
    }

    if (noteWindow.getSecondPreviousNote() != null && !noteWindow.getCurrentNote().isHarmonicNote()
      && (noteWindow.getPreviousNote() != null || noteWindow.getCurrentNote() != null)) {
    ...
    if (noteWindow.getPreviousNote() != null) {
      ...
      if (isLastSave && noteWindow.getPreviousNote().isFirstNote() && noteWindow.getCurrentNote()
        .isHarmonicNote()) {
      ...
      if (noteWindow.getPreviousNote().isHarmonicNote() && noteWindow.getCurrentNote()
        .isHarmonicNote()) {
        lyString.append(noteWindow.getPreviousNoteLy());
        lyString.append(" ");
      }
...
class NoteWindow {
  ...
  void addNote(Note note) {
    ...
    currentNoteDuration = LilyPondNoteDurationHelper
      .getDuration2(Double.toString(note.getDuration()));
  }
  ...
  /**
   * This method determines octave changes that are relative to the previous note.
   * <ul>
  ...
  int getOctaveChange(char currentNoteChar) {
    ...
    if (previousNote != null && !previousNote.isRest()) {
      lilypondChange = lilypondRelativeDirection(
        previousNote.originalString.toLowerCase().charAt(0), currentNoteChar);
    } else {
    ...
    int jfugueChange = 0;
    if (previousNote != null && !previousNote.isRest()) {
      jfugueChange = jfugueOctaveChange(previousNote.originalString.toLowerCase().charAt(0),
        currentNoteChar, lilypondChange);
    } else {
  ...
  void print() {
    log("Note window: " + secondPreviousNoteLy + "&" + secondPreviousNoteDuration + "&"
      + previousNoteLy + "&" + previousNoteDuration + "&" + currentNoteLy + "&"
      + currentNoteDuration);
  }
