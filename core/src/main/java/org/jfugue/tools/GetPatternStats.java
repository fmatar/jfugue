...
/**
 * Provides <code>Pattern</code> and MIDI analysis of the following elements: General descriptors,
 * Pitch descriptors, Duration descriptors, Silence descriptors (rests), Interval descriptors
 * (half-steps), Inter Onset Interval (IOI), Harmonic Descriptors (Non-diatonics), Rhythm
 * Descriptors (Syncopations), Normality Descriptors.
 *
...
public class GetPatternStats {
  ...
  private final List<Number> pitches = new ArrayList<>();
  private final List<Number> intervals = new ArrayList<>();
  private final List<Number> degreeNonDiatonic = new ArrayList<>();
  private final List<Number> interOI = new ArrayList<>();
  private final List<Number> durations = new ArrayList<>();
  private final List<Number> restDurations = new ArrayList<>();
  private final List<Byte> attacks = new ArrayList<>();
  private final List<Byte> decays = new ArrayList<>();
  private final List<TimeEvent> musicEvents = new ArrayList<>();
  ...
  /**
   * Parses JFugue <code>Pattern<code> to calculate statistics for all descriptors.
   *
   * @param pattern The JFuge Pattern to be parsed.
   * @param clear True to clear previous data, false to add to previous data
   */
  private void parsePattern(Pattern pattern, Boolean clear) {
    tickPos = 0;
    if (clear) {
      this.clearLists();
    }
   ...
   * @throws javax.sound.midi.InvalidMidiDataException - The file contains invalid midi
   */
  public void parsePattern(File midiFile, Boolean clear)
    throws IOException, InvalidMidiDataException {
    Pattern midiPattern = new Pattern();
    String ext = midiFile.getName()
      .substring(midiFile.getName().lastIndexOf(".") + 1);
  ...
  public double comparePatterns(Pattern p1, Pattern p2) {
    List<Number> difference = new ArrayList<>();
   ...
   */
  private Stats getPitchStats() {
    return new Stats(pitches);
  }

  /**
   * Gets <code>Stats</code> object containing note duration N, Average (mean - min), SD, and
   * Range.
   *
   * @return <code>Stats</code> object for note duration
   */
  private Stats getDurationStats() {
   ...
   */
  private Stats getRestStats() {
    return new Stats(restDurations);
  }

  /**
   * Gets <code>Stats</code> object containing pitch interval N, Average (mean - min), SD, and
   * Range. Intervals are in half-steps.
   *
   * @return <code>Stats</code> object for pitch interval
   */
  private Stats getIntervalStats() {
    return new Stats(intervals);
  }

  /**
   * Gets <code>Stats</code> object containing inter-onset-interval(IOI) N, Average (mean - min),
   * SD, and Range. Inter-onset-intervals are the number of MIDI pulses between the onset of
   * non-rest notes.
   *
   * @return <code>Stats</code> object for IOI
   */
  private Stats getIOIStats() {
    return new Stats(interOI);
  }

  /**
   * Gets number of syncopations. Syncopations are calculated as any note that begins between beats
   * and extends beyond the next beat. ie. C5H C5W C5H - the middle note is syncopated.
   *
   * @return <code>Stats</code> object for rhythm
   */
  private int getRhythmStats() {
    return rhythm;
  }

  /**
   * Gets <code>Stats</code> object containing harmonics N, Average (mean - min), SD, and Range.
   * Non-diatonic notes based off of the parsed key signature or default of Cmaj. Average is average
   * of 0-4, as a degree of non-diatonic notes. ie. 0: ♭II, 1: ♭III (♮III for minor key), 2: ♭V, 3:
   * ♭VI, 4: ♭VII. SD is also computed using degree of non-diatonic notes.
   *
   * @return <code>Stats</code> object for harmonics
   */
  private Stats getHarmonicStats() {
  ...
  private void sortTimeEvents() {
    musicEvents.sort(Comparator.comparingDouble(eg -> eg.time));
  }
  ...
  private boolean checkHarmonics(Note note) {
    ...
    majors.add(Arrays.asList(1, 3, 5, 5, 8, 10,
      0));   //Db Eb F Gb Ab Bb C 5 flats //C# D# E# F# G# A# B# 7 sharps
    ...
    majors.add(Arrays.asList(6, 8, 10, 11, 1, 3,
      5));  //Gb Ab Bb Cb Db Eb F 6 flats //F# G# A# B C# D# E# 6 sharps
    ...
    majors.add(Arrays.asList(11, 1, 3, 4, 6, 8,
      10));  //Cb Db Eb Fb Gb Ab Bb 7 flats //B C# D# E F# G# A# 5 sharps
    ...
    minors.add(Arrays
      .asList(6, 8, 10, 11, 1, 3, 5));  //Eb F Gb Ab Bb Cb Db 6 flats /D# E# F# G# A# B C# 6 sharps
    ...
    minors.add(Arrays
      .asList(11, 1, 3, 4, 6, 8, 10));  //G# A# B C# D# E F# 5 sharps /Ab Bb Cb Db Eb Fb Gb 7 flats
    minors.add(Arrays.asList(0, 2, 4, 5, 7, 9, 11));   // A B C D E F G no flats or sharps
    minors.add(Arrays.asList(1, 3, 5, 5, 8, 10,
      0));   //Bb C Db Eb F Gb Ab 5 flats /A# B# C# D# E# F# G# 7 sharps
    ...
    return !keyScale.contains(reduceValue(note.getValue()));
  }
  ...
  private void checkDegree(Number n) {
    int rootValue = reduceValue(key.getRoot().getValue());
  ...
  /**
   * Process all events in chronological order. - add pitch, duration, etc. to lists for further
  ...
  private class Listener implements ParserListener {
    ...
    public void onTrackChanged(byte t) {
      musicEvents.add(new TimeEvent<>(tickPos, t));
    }

    @Override
    public void onLayerChanged(byte layerNum) {
      musicEvents.add(new TimeEvent<>(tickPos, layerNum));
      tickPos = 0;
    }

    @Override
    public void onInstrumentParsed(byte i) {
      musicEvents.add(new TimeEvent<>(tickPos, i));
    }

    @Override
    public void onTempoChanged(int tBPM) {
      musicEvents.add(new TimeEvent<>(tickPos, tBPM));
    }
    ...
    public void onKeySignatureParsed(byte keyB, byte scale) {
      ...
      musicEvents.add(new TimeEvent<>(tickPos, k));
    }

    @Override
    public void onTimeSignatureParsed(byte bDuration, byte bNumber) {
      musicEvents.add(new TimeEvent<>(tickPos, new Byte[]{bDuration, bNumber}));
    }

    @Override
    public void onBarLineParsed(long m) {
      musicEvents.add(new TimeEvent<>(tickPos, m));
      ++measures;
    }
    ...
    public void onPitchWheelParsed(byte b, byte b1) {
      musicEvents.add(new TimeEvent<>(tickPos, new Byte[]{b, b1}));
    }

    @Override
    public void onChannelPressureParsed(byte b) {
      musicEvents.add(new TimeEvent<>(tickPos, b));
    }
    ...
    public void onLyricParsed(String lyric) {
      musicEvents.add(new TimeEvent<>(tickPos, lyric));
    }

    @Override
    public void onMarkerParsed(String string) {
      musicEvents.add(new TimeEvent<>(tickPos, string));
    }

    @Override
    public void onFunctionParsed(String string, Object o) {
      musicEvents.add(new TimeEvent<>(tickPos, o));
    }
    ...
    public void onNoteParsed(Note note) {
      musicEvents.add(new TimeEvent<>(tickPos, note));
    ...
    public void onChordParsed(org.jfugue.theory.Chord chord) {
      musicEvents.add(new TimeEvent<>(tickPos, chord));
