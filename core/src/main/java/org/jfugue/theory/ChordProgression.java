...
public class ChordProgression implements PatternProducer {
   ...
   *
   * @param progression a {@link java.lang.String} object.
   */
  public ChordProgression(String progression) {
    createProgression(progression.split(
      "[- ]")); // Split on either spaces or dashes // TODO Make ChordProgression parse on [- +]
  }

  /**
   * Creates a chord progression given an array of Progression Strings, like { "I", "vi", "ii", "V"
   * } - case is important!
   *
   * @param progressionElements an array of {@link java.lang.String} objects.
   */
   ...
   * <p>fromChords.</p>
   *
   * @param knownChords a {@link java.lang.String} object.
   * @return a {@link org.jfugue.theory.ChordProgression} object.
   */
   ...
   * <p>fromChords.</p>
   *
   * @param chords a {@link org.jfugue.theory.Chord} object.
   * @return a {@link org.jfugue.theory.ChordProgression} object.
   */
  ...
  public Pattern getPattern() {
    ...
    if (allSequence != null) {
      pattern = ReplacementFormatUtil
        .replaceDollarsWithCandidates(allSequence, getChords(), new Pattern(getChords()));
    }
    ...
    if (eachSequence != null) {
      ...
      for (String chordString : pattern.toString().split(" ")) { // TODO Should be " +"
        ...
        p2.add(ReplacementFormatUtil
          .replaceDollarsWithCandidates(eachSequence, chord.getNotes(), chord));
      }
      pattern = p2;
    }
   ...
   *
   * @return an array of {@link org.jfugue.theory.Chord} objects.
   */
  public Chord[] getChords() {
    ...
    for (String progressionElement : progressionElements) {
      ...
      if ((progressionElement.toLowerCase().indexOf("o") > 0) || (
        progressionElement.toLowerCase().indexOf("d") > 0)) {
  ...
  /**
   * Only converts Roman numerals I through VII, because that's all we need in music theory... VIII
   * would be the octave and equal I!
   */
  ...
  /**
   * Requires passing a string that has dollar signs followed by an index, in which case each
   * dollar+index will be replaced by the indexed note of the chord for each chord in the
   * progression. For example, given a ChordProgression of "I IV V" and a string of "$0q $1h $2w",
   * will return "Cq E4h G4w Fq A4h C5w Gq B4h D5w". Using the underscore character instead of an
   * index will result in the chord itself added to the string. The final result will be returned
  ...
  /**
   * Requires passing a string that has dollar signs followed by an index, in which case each
   * dollar+index will be replaced by the indexed chord of the chord progression. For example, given
   * a ChordProgression of "I IV V" and a string of "$0q $1h $2w", will return "C4MAJq F4MAJh
   * G4MAJw". Using the underscore character instead of an index will result in the pattern of the
   * ChordProgression itself added to the string. The final result will be returned from the
   * getPattern() method.
   *
   * @param sequence a {@link java.lang.String} object.
   * @return a {@link org.jfugue.theory.ChordProgression} object.
   ...
   * <p>distribute.</p>
   *
   * @param distribute a {@link java.lang.String} object.
   * @return a {@link org.jfugue.theory.ChordProgression} object.
   */
