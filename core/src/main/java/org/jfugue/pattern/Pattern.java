...
public class Pattern implements PatternProducer, TokenProducer {
  ...
  private StringBuilder patternSB;
   ...
   * <p>Constructor for Pattern.</p>
   *
   * @param producers a {@link org.jfugue.pattern.PatternProducer} object.
   */
   ...
   * <p>add.</p>
   *
   * @param producer a {@link org.jfugue.pattern.PatternProducer} object.
  ...
  /**
   * Prepends each producer in the order it is passed in, so if you pass in "F F", "G G", and "E E",
   * and the current pattern is "A A", you will get "F F G G E E A A".
   *
  ...
  /**
   * Inserts the given string to the beginning of this pattern. If there is content in this pattern
   * already, this method will insert a space between the given string and this pattern so the
   * tokens remain separate.
   */
  private Pattern prepend(String string) {
    if (patternSB.length() > 0) {
      patternSB.insert(0, " ");
    }
  ...
  public Pattern atomize() {
    ...
    currentLayer[valueOrZero(this.explicitVoice)] =
      "" + IVLSubparser.LAYER + valueOrZero(this.explicitLayer);
    currentInstrument[valueOrZero(this.explicitVoice)] =
      "" + IVLSubparser.INSTRUMENT + valueOrZero(this.explicitInstrument);
      ...
      switch (token.getType()) {
        case VOICE:
          ...
          if (currentLayer[voiceCounter] == null) {
            currentLayer[voiceCounter] = IVLSubparser.LAYER + "0";
          }
          if (currentInstrument[voiceCounter] == null) {
            currentInstrument[voiceCounter] = IVLSubparser.INSTRUMENT + "0";
          }
          break;
        case LAYER:
        ...
        case NOTE:
          this.add(
            new Atom(currentVoice, currentLayer[voiceCounter], currentInstrument[voiceCounter], s));
          break;
        default:
          this.add(s);
          break;
  ...
  /**
   * If the provided value is equal to UNDECLARED_EXPLICIT, return 0. Otherwise, return the value.
   */
  ...
  /**
   * Provides a way to explicitly set the tempo on a Pattern directly through the pattern rather
   * than by adding text to the contents of the Pattern.
   *
   * When Pattern.toString() is called, the a tempo will be prepended to the beginning of the
   * pattern in the form of "Tx", where x is the tempo number.
   *
   * @param explicitTempo The tempo
   * @return this pattern
   */
   ...
   *
   * When Pattern.toString() is called, the a tempo will be prepended to the beginning of the
   * pattern in the form of "Tx", where x is the tempo number (even though this method takes a
  ...
  /**
   * Provides a way to explicitly set the voice on a Pattern directly through the pattern rather
   * than by adding text to the contents of the Pattern.
   *
   * When Pattern.toString() is called, the a voice will be prepended to the beginning of the
   * pattern after any explicit tempo and before any explicit layer in the form of "Vx", where x is
   * the voice number
   *
   * @param voice The voice as an integer
   * @return this pattern
   */
  ...
  /**
   * Provides a way to explicitly set the layer on a Pattern directly through the pattern rather
   * than by adding text to the contents of the Pattern.
   *
   * When Pattern.toString() is called, the a layer will be prepended to the beginning of the
   * pattern after any explicit voice and before any explicit instrument in the form of "Lx", where
   * x is the voice number
   *
   * @param layer layer
   * @return this pattern
   */
  ...
  /**
   * Provides a way to explicitly set the instrument on a Pattern directly through the pattern
   * rather than by adding text to the contents of the Pattern.
   *
   * When Pattern.toString() is called, the a instrument will be prepended to the beginning of the
   * pattern after any explicit voice in the form of "I[instrument-name]" (even though this method
   ...
   *
   * When Pattern.toString() is called, the a instrument will be prepended to the beginning of the
   * pattern after any explicit voice in the form of "I[instrument-name]"
   *
   * @param instrument instrument
   * @return this pattern
   */
  ...
  /**
   * Expects a parameter of "note decorators" - i.e., things that are added to the end of a note,
   * such as duration or attack/decay settings; splits the given parameter on spaces and applies
   * each decorator to each note as it is encountered in the current pattern.
   *
   * If there is one decorator in the parameter, this method will apply that same decorator to all
   * note in the pattern.
   *
   * If there are more notes than decorators, a counter resets to 0 and the decorators starting from
   * the first are applied to the future notes.
   *
   * Examples:
   * <pre>
   ...
   * </pre>
   *
   * @param decoratorString decorator
   * @return this pattern
   */
  ...
  private Pattern save(File file, String... comments) throws IOException {
  ...
  /**
   * Parse this pattern and have the given ParserListener listen to it. The user will need to call a
   * function on the listener to get the result. This method returns its own class, the contents of
   * which ARE NOT modified by this call.
   *
   * @param listener the ParseListener interface
   * @return the resulting pattern
   */
