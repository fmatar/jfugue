...
public class SignatureSubparser implements Subparser, KeyProvider {
  ...
  /** Constant <code>KEY_SIGNATURE="KEY:"</code> */
  public static final String KEY_SIGNATURE = "KEY:";
  /** Constant <code>TIME_SIGNATURE="TIME:"</code> */
  ...
  private static final String[] MAJOR_KEY_SIGNATURES = new String[]{"Cb", "Gb", "Db", "Ab", "Eb",
    "Bb", "F", "C", "G", "D", "A", "E", "B", "F#", "C#"};
  private static final String[] MINOR_KEY_SIGNATURES = new String[]{"Ab", "Eb", "Bb", "F", "C", "G",
    "D", "A", "E", "B", "F#", "C#", "G#", "D#", "A#"};
  private static final int KEYSIG_MIDPOINT = 7;
  private static final String MAJOR_ABBR = "maj";
  private static final String MINOR_ABBR = "min";
  private static final char SHARP_CHAR = '#';
  private static final char FLAT_CHAR = 'B';
  private static SignatureSubparser instance;

  /**
   * <p>Getter for the field <code>instance</code>.</p>
   *
   * @return a {@link org.staccato.SignatureSubparser} object.
   */
  ...
  private boolean matchesKeySignature(String music) {
    return (music.length() >= KEY_SIGNATURE.length()) && (music.substring(0, KEY_SIGNATURE.length())
      .equals(KEY_SIGNATURE));
  }

  private boolean matchesTimeSignature(String music) {
    return (music.length() >= TIME_SIGNATURE.length()) && (music
      .substring(0, TIME_SIGNATURE.length()).equals(TIME_SIGNATURE));
  }
  ...
  public int parse(String music, StaccatoParserContext context) {
    if (matchesKeySignature(music)) {
      ...
      context.setKey(key);
      context.getParser().fireKeySignatureParsed(key.getRoot().getPositionInOctave(),
        key.getScale().getMajorOrMinorIndicator());
      ...
      byte denominator = Byte.parseByte(timeString.substring(posOfSlash + 1));
  ...
  public Key createKey(String keySignature) {
    ...
    if (keySignature.charAt(0) == 'K' && (keySignature.indexOf(SHARP_CHAR) == 1 || (
      keySignature.toUpperCase().indexOf(FLAT_CHAR) == 1))) {
  ...
  /**
   * Returns a Key given a string containing as many flats or sharps as the number one would see on
   * a staff for the corresponding key; e.g., "Kbbbb" = Ab Major
   */
  private Key createKeyFromAccidentals(String keySignature) {
    return new Key(
      MAJOR_KEY_SIGNATURES[KEYSIG_MIDPOINT + countAccidentals(keySignature)] + MAJOR_ABBR);
  }
  ...
  public byte convertAccidentalCountToKeyRootPositionInOctave(int accidentalCount, byte scale) {
    if (scale == Scale.MAJOR_INDICATOR) {
      return new Note(MAJOR_KEY_SIGNATURES[KEYSIG_MIDPOINT - accidentalCount])
        .getPositionInOctave();
    } else {
      return new Note(MINOR_KEY_SIGNATURES[KEYSIG_MIDPOINT - accidentalCount])
        .getPositionInOctave();
    }
  }
  ...
  public byte convertKeyToByte(Key key) {
    String noteName = Note.getDispositionedToneStringWithoutOctave(key.getScale().getDisposition(),
      key.getRoot().getValue());
    if (noteName == null) {
      return 0;
    }
    for (byte b = -KEYSIG_MIDPOINT; b < KEYSIG_MIDPOINT + 1; b++) {
      if (Note.isSameNote(noteName,
        (key.getScale() == Scale.MAJOR) ? MAJOR_KEY_SIGNATURES[KEYSIG_MIDPOINT + b]
          : MINOR_KEY_SIGNATURES[KEYSIG_MIDPOINT + b])) {
