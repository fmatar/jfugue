...
/**
 * <p>CollectedNotesPreprocessor class.</p>
 *
 * @author fmatar
 * @version $Id: $Id
 */
public class CollectedNotesPreprocessor implements Preprocessor {
  ...
  private static final Pattern parenPattern = Pattern.compile("\\([^)]*\\)\\S");
  private static CollectedNotesPreprocessor instance;

  /**
   * <p>Getter for the field <code>instance</code>.</p>
   *
   * @return a {@link org.staccato.CollectedNotesPreprocessor} object.
   */
  ...
  public String preprocess(String s, StaccatoParserContext context) {
    ...
    while (m.find()) {
      ...
      String replicand = s
        .substring(posCloseParen + 1, StaccatoUtil.findNextOrEnd(s, SPACE_PLUS, posCloseParen + 1));
      ...
      while (subindex < parenContents.length()) {
        ...
        buddy.append(parenContents, subindex, posSomething);
        buddy.append(replicand);
        if (posSomething != parenContents.length()) {
          buddy.append(parenContents, posSomething, posSomething + 1);
        }
        subindex = posSomething + 1;
      }
    }

    buddy.append(s.substring(posStart));
