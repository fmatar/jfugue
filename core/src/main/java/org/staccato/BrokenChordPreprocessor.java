...
/**
 * <p>BrokenChordPreprocessor class.</p>
 *
 * @author fmatar
 * @version $Id: $Id
 */
public class BrokenChordPreprocessor implements Preprocessor {
   ...
   *
   * @return a {@link org.staccato.BrokenChordPreprocessor} object.
   */
    ...
    for (String s : splitsville) {
      ...
      if (((posColon = s.indexOf(':')) != -1) && (posColon > 0)) {
        ...
        if (Chord.isValidChord(candidateChord)) {
          ...
          String dynamics = (posColon2 == s.length()) ? "" : s.substring(posColon2 + 1);
          ...
          if (replacementDescription.charAt(0) == '[') {
            String replacementLookup = replacementDescription
              .substring(1, replacementDescription.length() - 1);
            replacementDescription = (String) context.getDictionary().get(replacementLookup);
          }

          Map<String, PatternProducer> specialReplacers = new HashMap<>();
          ...
          Pattern result = ReplacementFormatUtil
            .replaceDollarsWithCandidates(replacementDescription, chord.getNotes(),
              wrapInParens(chord.getPatternWithNotes()), specialReplacers, ",", " ", dynamics);
