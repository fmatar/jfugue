...
public class BeatTimeSubparser implements Subparser {
  ...
  /** Constant <code>BEATTIME='@'</code> */
  public static final char BEATTIME = '@';
  /** Constant <code>BEATTIME_USE_MARKER='#'</code> */
   ...
   *
   * @return a {@link org.staccato.BeatTimeSubparser} object.
   */
    ...
    if (music.charAt(0) == BEATTIME) {
      ...
      if (posNextSpace > 1) {
        ...
        if (timeTrackId.matches("([0-9]+(\\.[0-9]+)*)")) {
          ...
          String timeBookmarkId = timeTrackId.substring(1);
