...
/**
 * The MicrotonePreprocess lets a user express a microtone using 'm' followed by the frequency -
 * e.g., m440. The MicrotonePreprocessor takes this String, parses the frequency value, figures out
 * what Pitch Wheel and Note events need to be called to generate this frequency in MIDI, and
...
public class MicrotonePreprocessor implements Preprocessor {
  ...
  private static final Pattern microtonePattern = Pattern.compile("(^|\\s)[Mm]\\S+");
  private static final Pattern frequencyPattern = Pattern.compile("[0-9.]+");
  private static final Pattern qualifierPattern = Pattern
    .compile("[WHQISTXOADwhqistxoad/]+[0-9.]*\\S*");
  private static MicrotonePreprocessor instance;

  /**
   * <p>Getter for the field <code>instance</code>.</p>
   *
   * @return a {@link org.staccato.MicrotonePreprocessor} object.
   */
  ...
  public static String convertFrequencyToStaccato(double frequency, String qualifier) {
    ...
    if (pitches > 0) {
      buddy.append(
        " :PitchWheel(8192)"); // Reset the pitch wheel.  8192 = original pitch wheel position
    }
    ...
    while (m.find()) {
      ...
      if (frequencyMatcher.find()) {
        ...
        throw new IllegalArgumentException(
          "The following is not a valid microtone frequency: " + frequencyMatcher.group());
      }
