...
/**
 * <p>StaccatoElementSorter class.</p>
 *
 * @author fmatar
 * @version $Id: $Id
 */
public class StaccatoElementSorter extends TrackTimeManager implements ParserListener {
  ...
  private final Map<Double, List<ElementWithTrack>> timeMap;
  private double initialNoteBeatTime;

  private StaccatoElementSorter() {
    super();
    timeMap = new HashMap<>();
  }
   ...
   *
   * @param patternProducer The pattern producer
   * @return A map of sorted elements
   */
   ...
   */
  private Map<Double, List<ElementWithTrack>> getSortedElements() {
    return timeMap;
  }
  ...
  private void addToTimeMap(String string) {
    ElementWithTrack elementWithTrack = new ElementWithTrack(getCurrentTrackNumber(),
      getCurrentLayerNumber(), string);
    List<ElementWithTrack> elementList = timeMap
      .computeIfAbsent(getTrackBeatTime(), k -> new ArrayList<>());
