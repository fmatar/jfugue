...
public class NoteSubparser implements Subparser, NoteProvider, ChordProvider {
  ...
  private final List<Character> charArray = new ArrayList<>();
   ...
   *
   * @return a {@link org.staccato.NoteSubparser} object.
   */
...
//            }
   ...
   */
  private int parseNoteElement(String s, int index, NoteContext noteContext,
    StaccatoParserContext parserContext) {
    logger.info("--Parsing note from string " + s);
    s = s
      .toUpperCase(); // Ensure s is uppercase - which might not be the case if this is coming in via NoteProvider or ChordProvider
  ...
  private int parseLetterNote(String s, int index, NoteContext context) {
    ...
    logger.info(
      "Note number within an octave (C=0, B=11): " + context.noteNumber + " (with octaveBias = "
        + context.octaveBias + ")");
    return index;
  }
  ...
  private int parseBracketedNote(String s, int index, NoteContext context) {
    ...
    context.noteValueAsString = s.substring(index + 1, indexOfEndBracket);
    context.isNumericNote = true;

    logger
      .info("This note is a note represented by the dictionary value " + context.noteValueAsString);
  ...
  private int parseNumericNote(String s, int index, NoteContext context) {
    ...
    while (numCharsInNumber < s.length() && (s.charAt(index + numCharsInNumber) >= '0') && (
...
//            }
  ...
  private int parseInternalInterval(String s, int index, NoteContext context) {
    ...
    if ((index < s.length()) && (s.charAt(index) == '\'')) {
      ...
      if ((intervalLength == 1) && (index + 2 < s.length()) && isValidIntervalChar(
        s.charAt(index + 2))) {
        intervalLength = 2;
      }
      if ((intervalLength == 2) && (index + 3 < s.length()) && isValidIntervalChar(
        s.charAt(index + 3))) {
        intervalLength = 3;
      }
      context.internalInterval = Intervals
        .getHalfsteps(s.substring(index + 1, index + intervalLength + 1));
      context.originalString =
        Note.getToneStringWithoutOctave((byte) (context.noteNumber + context.internalInterval)) + (
          context.isOctaveExplicitlySet ? context.octaveNumber : "");
  ...
  private int parseChord(String s, int index, NoteContext context) {
    ...
    for (String chordName : chordNames) {
      if (!chordFound && (s.length() >= index + chordName.length()) && chordName
        .equals(s.substring(index, index + chordName.length()))) {
        chordFound = true;
        ...
        logger
          .info("Chord: " + chordName + "   Interval Pattern: " + Chord.getIntervals(chordName));
        break;
      }
    }
      ...
      if (index < s.length()) {
        switch (s.charAt(index)) {
          ...
          case '[':
            ...
            context.inversionBassNote = Note
              .getToneString(Byte.parseByte(s.substring(index + 1, indexEndBracket - 1)));
  ...
  private void computeNoteValue(NoteContext noteContext, StaccatoParserContext parserContext) {
    ...
    if (DefaultNoteSettingsManager.getInstance().getAdjustNotesByKeySignature()) {
      if (parserContext.getKey() != null) {
        ...
        if ((keySig != 0) && (!noteContext.isNatural)) {
          ...
          logger.info(
            "After adjusting for Key Signature, noteNumber=" + noteContext.noteNumber + " octave="
              + noteContext.octaveNumber);
        }
      }
    }
    ...
    if (!noteContext.isNumericNote) {
      ...
      if (intNoteNumber > 127) {
        throw new ParserException(StaccatoMessages.CALCULATED_NOTE_OUT_OF_RANGE,
   ...
   * done with its part
   */
  private int parseDuration(String s, int index, NoteContext noteContext,
    StaccatoParserContext parserContext) {
    if (index < s.length()) {
      switch (s.charAt(index)) {
        case '/':
        ...
        case '-':
          ...
          break;
        default:
          noteContext.decimalDuration = DefaultNoteSettingsManager.getInstance()
            .getDefaultDuration();
  ...
  private int parseQuantityDuration(String s, int index, NoteContext context) {
    ...
    context.decimalDuration +=
      (1.0f / context.mostRecentDuration) * (Double.parseDouble(quantityNumberString)
        - 1.0D); // Subtract 1, because mostRecentDuration has already been added to the total duration
    logger.info("Quantity duration calculation: Duration of 1/" + context.mostRecentDuration + " * "
      + quantityNumberString + " = " + (1.0 / context.mostRecentDuration) * Double
      .parseDouble(quantityNumberString));
    return endingIndex;
  }

  private int seekToEndOfDecimal(String s, int startingIndex) {
    int cursor = startingIndex;
    while (cursor < s.length() && (s.charAt(cursor) == '.' || ((s.charAt(cursor) >= '0') && (
      s.charAt(cursor) <= '9')))) {
      cursor++;
    }
    return cursor;
   ...
   * done with its part
   */
  private int parseLetterDuration(String s, int index, NoteContext context,
    StaccatoParserContext parserContext) {
    ...
    while (index < s.length()) {
      ...
      switch (velocityChar) {
        ...
        case 'D':
          ...
          break;
        default:
          throw new ParserException(StaccatoMessages.VELOCITY_CHARACTER_NOT_RECOGNIZED,
  ...
  class NoteContext {
    ...
    boolean isNumericNote;
    boolean isChord;
    String chordName;
    Intervals intervals;
    int inversionCount;
    String inversionBassNote;
    boolean isRest;
    boolean isNatural;
    boolean isOctaveExplicitlySet;
    int octaveBias;
    int octaveNumber;
    int internalInterval;
    double decimalDuration;
    String durationValueAsString;
    boolean isDurationExplicitlySet;
    int mostRecentDuration;
    boolean isEndOfTie;
    boolean isStartOfTie;
    boolean hasNoteOnVelocity;
    byte noteOnVelocity;
    String noteOnVelocityValueAsString;
    boolean hasNoteOffVelocity;
    byte noteOffVelocity;
    String noteOffVelocityValueAsString;
    boolean isFirstNote;
    boolean isMelodicNote;
    boolean anotherNoteIsMelodic;
    boolean isHarmonicNote;
    boolean anotherNoteIsHarmonic;
    boolean thereIsAnother;

    /**
     * NoteContext should only be constructed when a note token is first being parsed. Subsequent
     * parsings of notes within the same token must create a NoteContext using
     ...
     */
    NoteContext createNextNoteContext() {
      NoteContext noteContext = new NoteContext();
     ...
     * @return Note
     */
    Note createNote(StaccatoParserContext parserContext) {
      try {
     ...
     * @return Note
     */
    Chord createChord(StaccatoParserContext parserContext) {
      if (noteValueAsString != null) {
        noteNumber = (Byte) parserContext.getDictionary().get(noteValueAsString);
      }
      if (durationValueAsString != null) {
