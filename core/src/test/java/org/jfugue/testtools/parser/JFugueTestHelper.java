...
public class JFugueTestHelper {
  ...
  protected static final String TRACK_CHANGED = "trackChanged";
  protected static final String INSTRUMENT_PARSED = "instrumentParsed";
  protected static final String KEY_SIGNATURE_PARSED = "keySignatureParsed";
  protected static final String TIME_SIGNATURE_PARSED = "timeSignatureParsed";
  protected static final String PITCH_WHEEL_PARSED = "pitchWheelParsed";
  protected static final String POLYPHONIC_PRESSURE_PARSED = "polyphonicPressureParsed";
  protected static final String CONTROLLER_EVENT_PARSED = "controllerEventParsed";
  protected static final String CHANNEL_PRESSURE_PARSED = "channelPressureParsed";
  protected static final String LYRIC_PARSED = "lyricParsed";
  protected static final String MARKER_PARSED = "markerParsed";
  protected static final String TRACK_BEAT_TIME_BOOKMARK_REQUESTED = "trackBeatTimeBookmarkRequested";
  private static final String BEFORE_PARSING_STARTS = "beforeParsingStarts";
  private static final String AFTER_PARSING_FINISHED = "afterParsingFinished";
  private static final String LAYER_CHANGED = "layerChanged";
  private static final String TEMPO_CHANGED = "tempoChanged";
  private static final String BAR_LINE_PARSED = "barLineParsed";
  private static final String TRACK_BEAT_TIME_REQUESTED = "trackBeatTimeRequested";
  private static final String NOTE_PARSED = "noteParsed";
  private static final String CHORD_PARSED = "chordParsed";
  private static final String TRACK_BEAT_TIME_BOOKMARKED = "trackBeatTimeBookmarked";
  private static final String SYSEX_PARSED = "sysexParsed";
  private static final String USER_EVENT_PARSED = "userEventParsed";
  protected final StaccatoParser parser;
  private final ResultParserListener resultListener;

  protected JFugueTestHelper() {
  ...
  protected boolean compare() {
    parser.parse("{unknown instruction}");
    ArgEvent event = resultListener.getPenultimateEvent();
    return event.getEventName().equals(JFugueTestHelper.BEFORE_PARSING_STARTS);
  }
  ...
  protected boolean compare(double value) {
    ...
    return (event.getEventName().equals(JFugueTestHelper.TRACK_BEAT_TIME_REQUESTED) && (
  ...
  protected boolean compare(String stringToParse, String eventName, byte value1, byte value2) {
    ...
    return (event.getEventName().equals(eventName) && (event.getValue1() == value1) && (
      event.getValue2() == value2));
  }
  ...
  protected boolean compare(String stringToParse, String eventName, String id, Object object) {
    ...
    return (event.getEventName().equals(eventName) && (event.getId().equals(id)) && (event
      .getObject().equals(object)));
  }

  protected boolean compare(String stringToParse, byte[] value) {
    parser.parse(stringToParse);
    ByteArrayArgEvent event = (ByteArrayArgEvent) resultListener.getPenultimateEvent();
    return (event.getEventName().equals(JFugueTestHelper.SYSEX_PARSED) && (Arrays
      .equals(event.getValue(), value)));
  }

  protected boolean compare(String stringToParse, Note note) {
    parser.parse(stringToParse);
    ...
    if (event.getEventName().equals(JFugueTestHelper.NOTE_PARSED) && !(event.getNote()
      .equals(note))) {
    ...
    return (event.getEventName().equals(JFugueTestHelper.NOTE_PARSED) && (event.getNote()
      .equals(note)));
  }

  protected boolean compare(String stringToParse, Chord chord) {
    parser.parse(stringToParse);
    ChordArgEvent event = (ChordArgEvent) resultListener.getPenultimateEvent();
    // Only want this diagnostic stuff if you're testing whether event.getNote() equals Note (e.g., using assertTrue)
    if (event.getEventName().equals(JFugueTestHelper.CHORD_PARSED) && !(event.getChord()
      .equals(chord))) {
      System.out.println("Diagnostic info for " + stringToParse + ":");
    ...
    return (event.getEventName().equals(JFugueTestHelper.CHORD_PARSED) && event.getChord()
      .equals(chord));
  }

  class ResultParserListener implements ParserListener {

    private final List<ArgEvent> events;

    ResultParserListener() {
      this.events = new ArrayList<>();
    }

    /**
     * Returns the second-to-last event - because the last event is presumed to be
     * AFTER_PARSING_FINISHED
     */
    ArgEvent getPenultimateEvent() {
