...
public class ChordProgressionTest {
  ...
  public void testGetChords() {
    ...
    Chord[] checklist = new Chord[]{new Chord("A4maj"), new Chord("F#5min7"), new Chord("B4min"),
      new Chord("E5maj7")};
    for (int i = 0; i < chords.length; i++) {
      assertEquals(chords[i], checklist[i]);
    }
  }
  ...
  public void testEachChordAs() {
    ...
    assertEquals("F4q G#4q C5q G4q Bb4q D5q C4q Eb4q G4q", cp.getPattern().toString());

    cp = new ChordProgression("I IV V").eachChordAs("$0q $1h $2w");
    assertEquals("C4q E4h G4w F4q A4h C5w G4q B4h D5w", cp.getPattern().toString());
  }

  @Test
  public void testEachChordAsWithUnderscore() {
    ChordProgression cp = new ChordProgression("I IV V").eachChordAs("$!q $0q $1h $2w");
    assertEquals("C4MAJq C4q E4h G4w F4MAJq F4q A4h C5w G4MAJq G4q B4h D5w",
      cp.getPattern().toString());
  }

  @Test
  public void testAllChordsAs() {
    ChordProgression cp = new ChordProgression("iv v i").allChordsAs("$0q $1q $2q");
    assertEquals("F4MINq G4MINq C4MINq", cp.getPattern().toString());

    cp = new ChordProgression("I IV V").allChordsAs("$0q $1h $2w");
    assertEquals("C4MAJq F4MAJh G4MAJw", cp.getPattern().toString());
  }

  @Test
  public void testAllChordsAsWithBang() {
    ChordProgression cp = new ChordProgression("I IV V").allChordsAs("$!i $0q $1h $2w");
    assertEquals("C4MAJi F4MAJi G4MAJi C4MAJq F4MAJh G4MAJw", cp.getPattern().toString());

    cp = new ChordProgression("I IV V").allChordsAs("$0q $1h $2w $!i");
    assertEquals("C4MAJq F4MAJh G4MAJw C4MAJi F4MAJi G4MAJi", cp.getPattern().toString());
  }

  @Test
  public void testAllChordsAsWithInversions() {
    ChordProgression cp = new ChordProgression("iv v i")
      .allChordsAs("$0q $0^q $0^^q $1q $1^q $1^^q $2q $2^q $2^^q");
    assertEquals("F4MINq F4MIN^q F4MIN^^q G4MINq G4MIN^q G4MIN^^q C4MINq C4MIN^q C4MIN^^q",
      cp.getPattern().toString());
  }
  ...
  public void testAllChordsAsWithBangAndInversions() {
    ...
    assertEquals("F4MINq G4MINq C4MINq F4MIN^q G4MIN^q C4MIN^q F4MIN^^q G4MIN^^q C4MIN^^q",
      cp.getPattern().toString());
  }

  @Test
  public void testAllChordsAsAndEachChordAs() {
    ChordProgression cp = new ChordProgression("I IV V").allChordsAs("$2 $1 $0")
      .eachChordAs("$2 $1 $0");
    assertEquals("D5 B4 G4 C5 A4 F4 G4 E4 C4", cp.getPattern().toString());
  }

  @Test
  public void testChordProgressionWithInversions() {
    ChordProgression cp = new ChordProgression("I II^ III^^ IV^^^ v^^^ vi^^ vii^");
    assertEquals("C4MAJ D4MAJ^ E4MAJ^^ F4MAJ^^^ G4MIN^^^ A4MIN^^ B4MIN^",
      cp.getPattern().toString());
  }
