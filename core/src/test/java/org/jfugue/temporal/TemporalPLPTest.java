...
public class TemporalPLPTest {
  ...
  public void testTemporalTiming() throws MidiUnavailableException {
    ...
    final Map<Byte, TimeStruct> noteTimeTracker = new HashMap<>();
    ...
    for (Byte noteValue : noteTimeTracker.keySet()) {
      System.out.println("[TemporalPLPTest] For note value " + noteValue
        ...
        + ", timePlayedByTemporalPLP = " + noteTimeTracker
        .get(noteValue).timeNotePlayedByTemporalPLP
        + ", calculated delay = " + (noteTimeTracker.get(noteValue).timeNotePlayedByPlayer
        - noteTimeTracker.get(noteValue).timeNotePlayedByTemporalPLP)
  ...
  class TimeStruct {
    ...
    long timeNotePlayedByPlayer;
    long timeNotePlayedByTemporalPLP;
