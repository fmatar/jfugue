...
package org.jfugue.manualtest.realtime;
...
class RealtimeManualTest {

  private final JLabel volumeLabel = new JLabel();
  private final Pattern[] pattern = new Pattern[]{new Pattern("Ch Dh Eh"), new Pattern("Cq Dq Eq"),
    new Pattern("I[Piano] Cq I[Flute] Cq I[Piano]"), new Pattern("CmajW"),
    new Pattern("V0 Ch V1 Eh V2 Gh")};
  private org.jfugue.realtime.RealtimePlayer player;
  private JPanel panel;
  private byte volume = 60;

  private RealtimeManualTest() {
    try {
  ...
  private void init() {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      button.addActionListener(e -> playPattern(pattern[a]));
