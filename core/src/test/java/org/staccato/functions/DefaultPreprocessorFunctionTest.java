...
public class DefaultPreprocessorFunctionTest extends JFugueTestHelper {
  ...
  public void testDefaultOctave() {
    assertTrue(compare(":DEFAULT(OCTAVE=6) C", new Note(72)));
  }

  @Test
  public void testDefaultOctaveAndAttack() {
    assertTrue(compare(":DEFAULT(OCTAVE=6,ATTACK=60) C",
      new Note(72).setOnVelocity((byte) 60)));
  }

  @Test
  public void testDefaultAttackAndDecay() {
    assertTrue(compare(":DEFAULT(ATTACK=60,DECAY=80) C",
      new Note(60).setOnVelocity((byte) 60).setOffVelocity((byte) 80)));
  }

  @Test
  public void testDefaultDuration() {
    assertTrue(compare(":DEFAULT(DURATION=0.5) C",
      new Note(60).setImplicitDurationForTestingOnly(0.5)));
  }
  ...
  public void testDefaultBassOctave() {
    ...
    assertTrue(compare(":DEFAULT(BASS_OCTAVE=1) Cmaj", new Chord("CMAJ")));
  }

  @Test
  public void testCleanParse() {
    StaccatoParser parser = new StaccatoParser();
    assertEquals("C  D", parser.preprocess(new Pattern("C :DEFAULT(DURATION=0.25) D")));
