...
public class InstructionPreprocessorTest extends JFugueTestHelper {
  ...
  public void testSimpleInstruction() {
    ...
    assertTrue(compare("{letter}", new Note("C")));
  }
  ...
  public void testSwitchInstruction() {
    ...
    assertTrue(compare("{turn off}", new Note("C")));
    assertTrue(compare("{turn on}", new Note("D")));
  }
  ...
  public void testChoiceInstruction() {
    ...
    assertTrue(compare("{chorus 0}", new Note("C")));
    assertTrue(compare("{chorus 1}", new Note("D")));
    assertTrue(compare("{chorus 2}", new Note("E")));
  }
  ...
  public void testLastIsValueInstruction() {
    ...
    assertTrue(
      compare("{volume should now be set to the glorious value of 2}", CONTROLLER_EVENT_PARSED,
        (byte) 7, (byte) 2));
  }
  ...
  public void testLastIsValueToSplitInstruction() {
    InstructionPreprocessor ip = InstructionPreprocessor.getInstance();
    Instruction.Splitter splitter = value -> {
      Map<String, String> retVal = new HashMap<>();
      retVal.put("$1", "" + Integer.parseInt(value) / 128);
      retVal.put("$2", "" + Integer.parseInt(value) % 128);
      return retVal;
    };
    ip.addInstruction("set total volume",
      new Instruction.LastIsValueToSplit(":CON(7,$1) :CON(39,$2)", splitter));
    assertTrue(
      compare("{set total volume to 1600}", CONTROLLER_EVENT_PARSED, (byte) 39, (byte) 64));
  }
  ...
  public void testPatternAsInstruction() {
    ...
    assertTrue(compare("{pattern}", new Note("C")));
  }

  @Test
  public void testUnknownInstruction() {
    assertTrue(compare(
    )); // Remember that compare is looking for the penultimate event - the one *before* AFTER_PARSING_FINISHES
  }
  ...
  public void testInstructionsBySize() {
    ...
    ip.addInstruction("life is a lemon and i want my money back",
      new Instruction.LastIsValue(":CON(7,$)"));
    ip.addInstruction("life is a lemon", new Instruction.LastIsValue(":CON(8,$)"));
    assertTrue(compare("{life is a lemon and i want my money back because meat loaf said so 42}",
      CONTROLLER_EVENT_PARSED, (byte) 7, (byte) 42));
  }
}
