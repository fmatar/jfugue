...
class MicrotoneManualTest {
  ...
  public static void main(String[] args) {
    ...
    for (double freq = Note.getFrequencyForNote("A4"); freq < Note.getFrequencyForNote("C6");
      freq += 10.5) {
    ...
    ManualTestPrint.step(
      "Second, playing microtone scale. You should hear middle ranges in these notes: "
        + microtoneScale);
    player.play(microtoneScale);

    ManualTestPrint.step(
      "The expanded version of the microtone string is: " + MicrotonePreprocessor.getInstance()
        .preprocess(microtoneScale.toString(), null));

    player.play("Rw5");

    Pattern pitchWheelDemo = new Pattern();
    ManualTestPrint.step(
      "Now, let's do a pitch wheel demo. Same note (Middle-C, note 60), played with 163 different pitch wheel settings, each 100 cents apart.");
    ...
    ManualTestPrint.step(
      "Now playing four notes. The first two should sound different from each other, and the second two should sound the same as the first two.");
